...
// read the data file
function readData(){
    let dataRead = fs.readFileSync('./data/data.json');
...
function combineCounts(name, value){
    ...
    for (var i=0; i<info[name].length; i++){
        ...
        if (info[name][i][name] === value){
            info[name][i].count = parseInt(info[name][i].count) + 1;
            found = 1;
        }
    }    
    if (found === 0){
        info[name].push({[name] : value, count: 1});
    }
    writeData(info);
}
...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var data = readData();
        res.render('showResults', {results: [data.color, data.fruit, data.animal,data.chocolateBrand,data.Locorice,data.videoGame]});
        console.log([data.color, data.fruit, data.animal,data.chocolateBrand,data.Locorice,data.videoGame]);
    });
